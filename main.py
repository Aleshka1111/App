...
from track_loader import get_audio_info, update_tracks_json
...
class MusicApp(App):
    def build(self):
        ...
        self.progress_width = dp(300)
        self.progress_height = self.bar_height
    ...
    def draw_prev_icon(self, *args):
        if not self.nav_texture:
            return
        self.prev_btn.canvas.before.clear()
        with self.prev_btn.canvas.before:
            PushMatrix()
            if self.prev_btn.width > 1 and self.prev_btn.height > 1:
                Rotate(angle=180, origin=self.prev_btn.center)
            Rectangle(
                texture=self.nav_texture,
                pos=self.prev_btn.pos,
                size=self.prev_btn.size
            )
            PopMatrix()
    ...
    def on_progress_touch(self, widget, touch):
        ...
        if x <= touch.x <= x + self.progress_width and y <= touch.y <= bar_y:
            ...
            if self.player:
                duration = self.player.get_metadata().get('duration', 1)
                self.player.seek(progress * duration)
            return True
        return False
    ...
    def check_end(self, dt):
        if self.is_playing and self.player:
            duration = self.player.get_metadata().get('duration', 1)
            if self.player.get_pts() >= duration - 0.5:
    ...
    def load_image(self, paths):
        for path in paths:
            if os.path.exists(path):
                try:
                    img = CoreImage(path)
                    if img.texture: return img.texture
    ...
    def add_track_to_json(self, title, length):
        ...
        if os.path.exists("tracks.json"):
            with open("tracks.json", "r", encoding="utf-8") as f:
                tracks = json.load(f)
        tracks[title] = {"len_song": length, "filepath": f"music/{title}{os.path.splitext(self.selected_file_path)[1]}"}
